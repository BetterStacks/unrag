...
describe("google-drive connector: mime routing", () => {
  test("classifyDriveMimeType recognizes folders and google-native types", () => {
    expect(classifyDriveMimeType(DRIVE_MIME.folder)).toEqual({ kind: "folder" });
    expect(classifyDriveMimeType(DRIVE_MIME.shortcut)).toEqual({ kind: "shortcut" });
    expect(classifyDriveMimeType(DRIVE_MIME.doc)).toEqual({
      kind: "google_native",
      nativeKind: "doc",
    });
    expect(classifyDriveMimeType("application/pdf")).toEqual({ kind: "binary" });
  });
...
describe("google-drive connector: auth normalization", () => {
  ...
  test("oauth: validates required config fields", () => {
    expect(() =>
      normalizeGoogleDriveAuth({ kind: "oauth" } as any)
    ).toThrow();
