...
describe('unrag add dropbox', () => {
	...
	test('installs connector files and updates config', async () => {
		...
		await addCommand(['dropbox', '--yes', '--no-install'])

		expect(
			await pathExists(
				path.join(runDir, 'lib/unrag/connectors/dropbox/index.ts')
			)
		).toBe(true)
		...
		expect(cfg.connectors).toEqual(['dropbox'])
	})
})
