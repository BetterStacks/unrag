...
describe('unrag@latest init', () => {
	...
	test('installs drizzle adapter and merges deps', async () => {
		...
		expect(await pathExists(path.join(runDir, 'unrag.config.ts'))).toBe(
			true
		)
		expect(
		...
		expect(
			await pathExists(path.join(runDir, 'lib/unrag/core/deep-merge.ts'))
		).toBe(true)

		// Vendored sources should not retain internal monorepo alias imports.
		const ingest = await readFile(
			path.join(runDir, 'lib/unrag/core/ingest.ts'),
			'utf8'
		)
	...
	test('detects Next and patches tsconfig paths', async () => {
		...
		const tsconfig = await readJson<TsConfigJson>(
		...
		expect(tsconfig.compilerOptions.paths['@unrag/config']).toEqual([
			'./unrag.config.ts'
		])
	})
	...
	test('supports custom alias base', async () => {
		...
		expect(tsconfig.compilerOptions.paths['@rag/config']).toEqual([
			'./unrag.config.ts'
		])
	...
	test('installs batteries from preset (eval) as part of init', async () => {
		...
		const runner = await readFile(
			path.join(runDir, 'lib/unrag', 'eval', 'runner.ts'),
			'utf8'
		)
...
describe('unrag init - TypeScript import alias (non-Next.js)', () => {
	...
	test('supports custom alias base for plain TypeScript project', async () => {
		...
		expect(tsconfig.compilerOptions.paths['@myrag/config']).toEqual([
			'./unrag.config.ts'
		])
