...
describe('unrag add notion', () => {
	...
	test('installs connector files and merges deps', async () => {
		...
		expect(
			await pathExists(
				path.join(runDir, 'lib/unrag/connectors/notion/index.ts')
			)
		).toBe(true)
		...
		expect(pkg.dependencies?.['@notionhq/client']).toBeTruthy()

		const cfg = await readJson<{connectors?: string[]}>(
			path.join(runDir, 'unrag.json')
		)
		expect(cfg.connectors).toEqual(['notion'])
	})
})
