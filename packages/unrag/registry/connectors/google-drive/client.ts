 ...
 */
function isServiceAccountAuth(
	auth: GoogleDriveAuth
): auth is GoogleDriveServiceAccountAuth {
...
function isGoogleAuth(
	auth: GoogleDriveAuth
): auth is GoogleDriveGoogleAuthAuth {
...
export function normalizeGoogleDriveAuth(
	auth: GoogleDriveAuth
): NormalizedAuth {
	...
	if (isOAuthAuth(auth)) {
		...
		const {clientId, clientSecret, redirectUri, refreshToken, accessToken} =
			auth
	...
	throw new Error(
		`Unknown Google Drive auth kind: ${String((auth as Record<string, unknown>).kind)}`
	)
}
...
export async function createGoogleDriveClient(args: {
	...
	const scopes = (
		args.scopes?.length ? args.scopes : DEFAULT_DRIVE_SCOPES
	) as string[]
	...
	try {
		if (normalized.kind === 'oauth_client') {
			...
			const gal = (await import(
				'google-auth-library'
			)) as GoogleAuthLibraryModule

			if (normalized.kind === 'oauth_config') {
				const OAuth2Client = gal.OAuth2Client ?? gal.OAuth2
				if (!OAuth2Client) {
					throw new Error(
						'OAuth2Client not found in google-auth-library'
					)
				}
