...
export const ingest = async (
	...
	const baseTextChunks = await config.chunker(input.content, chunkingOptions)
		...
		for (const ex of args.extractors) {
			...
			try {
				...
				for (const item of nonEmptyItems) {
					const chunks = await config.chunker(
						item.content,
						chunkingOptions
					)
						...
						outSpecs.push({
							...
							metadata: {
								...
								...(item.confidence !== undefined
									...
									: {}),
								...(item.pageRange
									? {extractorPageRange: item.pageRange}
									: {}),
								...(item.timeRangeSec
									? {extractorTimeRangeSec: item.timeRangeSec}
									: {})
							},
							embed: {kind: 'text', text: c.content},
							storedContent: storeChunkContent ? c.content : '',
							storedTokenCount: storeChunkContent
								? c.tokenCount
								: 0
						})
					}
				...
				outWarnings.push({
					...
					...(args.assetMediaType
						? {assetMediaType: args.assetMediaType}
						: {})
				})
	...
	const processAsset = async (
		...
		const assetUri =
			asset.uri ??
			(asset.data.kind === 'url' ? asset.data.url : undefined)
		...
		if (asset.kind === 'image') {
			...
			if (config.embedding.embedImage) {
				...
				const captionChunks = await config.chunker(
					caption,
					chunkingOptions
				)
	...
	if (imageSpecs.length > 0) {
		...
		if (!embedImage) {
			throw new Error(
				'Image embedding requested but provider does not support embedImage()'
			)
		}
	...
	for (let i = 0; i < embeddedChunks.length; i++) {
		if (!embeddedChunks[i]) {
			throw new Error(
				'Internal error: missing embedding for one or more chunks'
			)
		}
	}
