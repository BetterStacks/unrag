...
export async function writeEvalReport(outputDir: string, report: EvalReportV1): Promise<string> {
...
export async function writeEvalSummaryMd(outputDir: string, report: EvalReportV1): Promise<string> {
  await ensureDir(outputDir);
  ...
  const stageLines = (label: string, a: EvalAggregatesForStage) => {
    ...
    lines.push(`| --- | ---: | ---: |`);
    lines.push(`| hit@k | ${a.mean.hitAtK.toFixed(3)} | ${a.median.hitAtK.toFixed(3)} |`);
    lines.push(`| recall@k | ${a.mean.recallAtK.toFixed(3)} | ${a.median.recallAtK.toFixed(3)} |`);
    lines.push(`| precision@k | ${a.mean.precisionAtK.toFixed(3)} | ${a.median.precisionAtK.toFixed(3)} |`);
    lines.push(`| mrr@k | ${a.mean.mrrAtK.toFixed(3)} | ${a.median.mrrAtK.toFixed(3)} |`);
    if (report.config.includeNdcg) {
      lines.push(`| ndcg@k | ${(a.mean.ndcgAtK ?? 0).toFixed(3)} | ${(a.median.ndcgAtK ?? 0).toFixed(3)} |`);
    }
    lines.push(``);
  };

  stageLines("Retrieved", report.results.aggregates.retrieved);
  if (report.results.aggregates.reranked) stageLines("Reranked", report.results.aggregates.reranked);
  ...
  const worst = [...report.results.queries].sort((a, b) => sortKey(a) - sortKey(b)).slice(0, 10);
  ...
  for (const q of worst) {
    ...
    lines.push(`| \`${q.id}\` | ${m.recallAtK.toFixed(3)} | ${m.hitAtK.toFixed(0)} | ${m.mrrAtK.toFixed(3)} |`);
  }
  lines.push(``);

  if (Array.isArray(report.results.thresholdFailures) && report.results.thresholdFailures.length > 0) {
...
export async function readEvalReportFromFile(reportPath: string): Promise<EvalReportV1> {
  ...
  try {
    ...
    throw new Error(`[unrag:eval] Failed to parse report JSON (${reportPath}): ${msg}`);
  }
  if (!json || typeof json !== "object") {
    throw new Error(`[unrag:eval] Invalid report JSON (${reportPath}): must be an object`);
  }
  const r = json as any;
  if (r.version !== "1") {
    throw new Error(`[unrag:eval] Unsupported report version (${reportPath}): ${String(r.version)}`);
  }
...
export function diffEvalReports(args: {
  ...
  const deltaStage = (bm: EvalMetricsAtK, cm: EvalMetricsAtK): Partial<EvalMetricsAtK> => ({
  ...
  const retrieved = deltaStage(b.results.aggregates.retrieved.mean, c.results.aggregates.retrieved.mean);

  const reranked =
    b.results.aggregates.reranked && c.results.aggregates.reranked
      ? deltaStage(b.results.aggregates.reranked.mean, c.results.aggregates.reranked.mean)
      : undefined;

  const p95TotalMs = c.results.timings.totalMs.p95 - b.results.timings.totalMs.p95;
  ...
  const ids = Array.from(new Set([...baselineById.keys(), ...candidateById.keys()]));
  ...
  return {
    ...
    baseline: { reportPath: args.baselinePath, datasetId: b.dataset.id, createdAt: b.createdAt },
    candidate: { reportPath: args.candidatePath, datasetId: c.dataset.id, createdAt: c.createdAt },
    deltas: { retrieved, ...(reranked ? { reranked } : {}), p95TotalMs },
    worstRegressions: regressions,
  };
}

export async function writeEvalDiffJson(outputDir: string, diff: EvalDiffV1): Promise<string> {
  await ensureDir(outputDir);
...
export async function writeEvalDiffMd(outputDir: string, diff: EvalDiffV1): Promise<string> {
  await ensureDir(outputDir);
