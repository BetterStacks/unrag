...
export function parseEvalDataset(json: unknown): EvalDatasetV1 {
	...
	const rerankTopK = asOptionalNumber(
		defaults.rerankTopK,
		'$.defaults.rerankTopK'
	)
	const mode =
		defaults.mode === undefined
			...
			: ((): EvalMode => {
					...
					if (v !== 'retrieve' && v !== 'retrieve+rerank') {
						throw err(
							'$.defaults.mode',
							'must be "retrieve" or "retrieve+rerank"'
						)
					}
					return v
				})()
	...
	const documents = (() => {
		...
		for (let i = 0; i < json.documents.length; i++) {
			...
			const content =
				d.content === undefined
					? undefined
					: asNonEmptyString(d.content, `$.documents[${i}].content`)
			const loaderRef =
				d.loaderRef === undefined
					? undefined
					: asNonEmptyString(
							d.loaderRef,
							`$.documents[${i}].loaderRef`
						)
			if (!content && !loaderRef) {
				throw err(
					`$.documents[${i}]`,
					'must include "content" or "loaderRef"'
				)
			}
			const metadata =
	...
	for (let i = 0; i < json.queries.length; i++) {
		...
		const qScopePrefix =
			q.scopePrefix === undefined
				? undefined
				: asNonEmptyString(q.scopePrefix, `$.queries[${i}].scopePrefix`)
		const qRerankTopK = asOptionalNumber(
			q.rerankTopK,
		...
		const notes =
			q.notes === undefined
				? undefined
				: asNonEmptyString(q.notes, `$.queries[${i}].notes`)
		queries.push({
			id: qid,
			query,
			topK: qTopK,
...
export async function readEvalDatasetFromFile(
	datasetPath: string
): Promise<EvalDatasetV1> {
	...
	try {
		...
		throw new Error(
			`[unrag:eval] Failed to parse dataset JSON (${datasetPath}): ${msg}`
		)
	}
