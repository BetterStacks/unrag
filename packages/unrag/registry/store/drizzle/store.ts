...
export const createDrizzleVectorStore = (db: DrizzleDb): VectorStore & { inspector: DebugStoreInspector } => {
  const inspector: DebugStoreInspector = {
    listDocuments: async ({ prefix, limit = 50, offset = 0 }) => {
      const where = prefix ? sql`where d.source_id like ${prefix + "%"}` : sql``;
      ...
      const docRows = (Array.isArray(rows) ? (rows as any[]) : ((rows as any)?.rows ?? [])) as Array<{
        source_id: unknown;
        created_at: unknown;
        chunk_count: unknown;
      }>;

      const totalRows = (Array.isArray(totalRes)
        ? (totalRes as any[])
        : ((totalRes as any)?.rows ?? [])) as Array<{ total: unknown }>;
    ...
    getDocument: async ({ sourceId }) => {
      ...
      const docRows = (Array.isArray(docRes)
        ? (docRes as any[])
        : ((docRes as any)?.rows ?? [])) as Array<{ id: unknown; source_id: unknown; metadata: unknown }>;
      ...
      const chunkRows = (Array.isArray(chunksRes)
        ? (chunksRes as any[])
        : ((chunksRes as any)?.rows ?? [])) as Array<{
        id: unknown;
        idx: unknown;
        content: unknown;
        metadata: unknown;
    ...
    storeStats: async () => {
      ...
      const rows = (Array.isArray(res) ? (res as any[]) : ((res as any)?.rows ?? [])) as Array<{
