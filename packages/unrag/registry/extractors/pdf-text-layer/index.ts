...
async function getPdfJsAssetUrls(): Promise<{
  ...
  cachedPdfJsAssetUrls = {
    standardFontDataUrl: pathToFileURL(withSlash(path.join(root, "standard_fonts")))
      .href,
...
export function createPdfTextLayerExtractor(): AssetExtractor {
  return {
    ...
    extract: async ({ asset, ctx }) => {
      ...
      const maxPages = Math.max(0, Math.min(totalPages, cfg.maxPages ?? totalPages));
      ...
      let out = "";
      outer: for (let start = 1; start <= maxPages; start += pageConcurrency) {
