...
export function Doctor({connection}: DoctorProps) {
	...
	const listViewportRows = Math.max(
		8,
		Math.min(26, rows - (canSplit ? 16 : 18))
	)
	const scroll = useScrollWindow({
		...
		resetKey: String(
			res?.type === 'doctor' && res.success ? res.checks.length : 0
		)
	})
	useInput((input, key) => {
		if (input === 'r') {
			...
			>
				...
				>
					<Box justifyContent="space-between" marginBottom={1}>
						...
						<Text color={theme.muted}>
							{summary.ok} ok · {summary.warn} warn ·{' '}
							{summary.err} err
						</Text>
					</Box>
					...
					<Box flexDirection="column" gap={1}>
						<Box flexDirection="column">
							...
							<Box
								...
								borderColor={
									selectedCheck
										? statusColor(selectedCheck.status)
										: theme.border
								}
								paddingX={1}
							>
						...
						{info?.runtime && (
							<Box flexDirection="column" gap={1}>
								...
								<Text color={theme.fg}>
									registered:{' '}
									<Text bold>
										{String(info.runtime.registered)}
									</Text>{' '}
									· engine:{' '}
									<Text bold>
										{String(info.runtime.hasEngine)}
									</Text>{' '}
									· storeInspector:{' '}
									<Text bold>
										{String(info.runtime.hasStoreInspector)}
									</Text>
								</Text>
						...
						{engineInfo && (
							<Box flexDirection="column" gap={1}>
								...
								<Text color={theme.fg}>
									embedding:{' '}
									<Text bold>
										{engineInfo.embedding.name}
									</Text>
									{engineInfo.embedding.dimensions
										? ` · dim ${engineInfo.embedding.dimensions}`
										: ''}{' '}
									· batch{' '}
									<Text bold>
										{String(
											engineInfo.embedding.supportsBatch
										)}
									</Text>{' '}
									· image{' '}
									<Text bold>
										{String(
											engineInfo.embedding.supportsImage
										)}
									</Text>
								</Text>
								<Text color={theme.fg}>
									storage: chunkContent{' '}
									<Text bold>
										{String(
											engineInfo.storage.storeChunkContent
										)}
									</Text>{' '}
									· docContent{' '}
									<Text bold>
								...
								<Text color={theme.fg}>
									chunking: size{' '}
									<Text bold>
										{engineInfo.defaults.chunkSize}
									</Text>{' '}
									· overlap{' '}
									<Text bold>
										{engineInfo.defaults.chunkOverlap}
									</Text>
								</Text>
								<Text color={theme.fg}>
									extractors:{' '}
									<Text bold>
										{engineInfo.extractorsCount}
									</Text>
								</Text>
								<Text color={theme.fg}>
									reranker:{' '}
									<Text bold>
										{engineInfo.rerankerName ?? 'none'}
									</Text>
								</Text>
							</Box>
						)}
