				...
				<Box gap={1}>
					...
					{queryCapable ? (
						...
						<Text color={theme.warning}>
							{chars.cross} Server does not advertise query
							capability. Register engine with{' '}
			...
			>
				<Box flexDirection="column">
					<Box gap={1}>
						...
						<Text
							color={mode === 'editing' ? theme.accent : theme.fg}
							bold={mode === 'editing'}
						>
							{query || ' '}
						</Text>
			...
			>
				...
				>
					<Box marginBottom={1} justifyContent="space-between">
						<Box gap={1}>
							...
							{chunks.length > 0 && (
								<Text color={theme.muted}>
									{scroll.windowStart + 1}-{scroll.windowEnd}{' '}
									of {chunks.length}
								</Text>
							)}
						</Box>
					...
					{chunks
						...
						.map((c, idx) => {
							...
							const docW = 10
							let contentW = clamp(
								Math.floor(resultsInnerWidth * 0.46),
								22,
								canSplit ? 80 : 44
							)
							let sourceW =
								resultsInnerWidth -
								(2 + scoreW + docW + contentW + 4)
							if (sourceW < 16) {
							...
							return (
								<Box key={`${c.id}-${i}`} gap={1}>
									...
									<Box width={docW} flexShrink={0}>
										<Text color={theme.muted}>
											{truncate(c.documentId, docW)}
										</Text>
									</Box>
									<Box
										width={Math.max(16, contentW)}
										flexShrink={0}
									>
										<Text color={theme.fg}>
											{truncate(
												content,
												Math.max(16, contentW)
											)}
										</Text>
									</Box>
								</Box>
				...
				>
					<Box marginBottom={1} gap={1}>
						...
						<Text color={theme.muted}>
							{selected ? truncate(selected.id, 14) : '—'}
						</Text>
					</Box>
