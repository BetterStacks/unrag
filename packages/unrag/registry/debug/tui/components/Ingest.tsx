...
function canIngest(connection: DebugConnection): boolean {
  return Array.isArray(connection.capabilities) && connection.capabilities.includes("ingest");
}
...
export function Ingest({ connection }: IngestProps) {
  ...
  const metaParse = useMemo(() => {
    ...
    if (!t) return { ok: true as const, value: undefined as Metadata | undefined };
    try {
      ...
      const cmd =
        inputMode === "file"
          ? {
              ...
              ...(metaParse.ok && metaParse.value ? { metadata: metaParse.value } : {}),
              chunking: {
  ...
  useInput((input, key) => {
    ...
    if (mode === "editing") {
      ...
      if (backspace) {
        ...
        if (focus === "content") {
          if (inputMode === "file") setContentPath((s) => s.slice(0, -1));
          else setContent((s) => s.slice(0, -1));
        }
        if (focus === "metadata") setMetadataText((s) => s.slice(0, -1));
        if (focus === "chunkSize") setChunkSize((n) => (typeof n === "number" ? Number(String(n).slice(0, -1)) || 0 : undefined));
        if (focus === "chunkOverlap")
          setChunkOverlap((n) => (typeof n === "number" ? Number(String(n).slice(0, -1)) || 0 : undefined));
        return;
      }
      ...
      if (input && input.length === 1 && !key.ctrl && !key.meta) {
        ...
        if (focus === "chunkSize") setChunkSize((n) => Number(`${typeof n === "number" ? n : ""}${input}`));
        if (focus === "chunkOverlap") setChunkOverlap((n) => Number(`${typeof n === "number" ? n : ""}${input}`));
        return;
      }
    ...
    if (key.tab) {
      const order: Field[] = ["sourceId", "content", "metadata", "chunkSize", "chunkOverlap"];
    ...
    const hasWarnings = Boolean(out?.success && out?.warnings && out.warnings.length > 0);
      ...
      <Box justifyContent="space-between" marginBottom={1}>
        <Box gap={1}>
          ...
          <Text color={theme.muted}>
            {mode === "editing" ? "editing: type · esc/^x exit · ⏎ apply" : "tab field · e edit · m mode · r run · c clear"}
          </Text>
        </Box>
        <Text color={theme.muted}>
          mode <Text color={theme.fg} bold>{inputMode}</Text>
        </Text>
      </Box>
      ...
      {!ingestCapable ? (
        ...
        <>
          ...
          >
            ...
            {field("metadata", metadataText.trim() ? metadataText : "—", focus === "metadata")}
            {field("chunkSize", typeof chunkSize === "number" ? String(chunkSize) : "default", focus === "chunkSize")}
            {field("chunkOverlap", typeof chunkOverlap === "number" ? String(chunkOverlap) : "default", focus === "chunkOverlap")}
            {!metaParse.ok && (
              <Text color={theme.error} bold>
                {chars.cross} metadata invalid: {metaParse.error}
              </Text>
            )}
            {inputMode === "inline" && (
              <Text color={theme.muted}>
                tip: inline content is single-line; use file mode for large/multiline docs
              </Text>
            )}
          </Box>
            ...
            <Box borderStyle="round" borderColor={theme.ok} paddingX={1} paddingY={1}>
              <Box gap={2} flexWrap="wrap">
                ...
                <Text color={theme.fg} bold>
                  {out.documentId ? truncate(out.documentId, 20) : "—"}
                </Text>
          ...
          {ok && out?.warnings && out.warnings.length > 0 && (
            <Box marginTop={1}>
              <Text color={theme.muted}>warnings ({out.warnings.length})</Text>
              <ScrollableText
                text={out.warnings.map((w) => `${w.code}: ${w.message}`).join("\n")}
                width={detailsWidth}
