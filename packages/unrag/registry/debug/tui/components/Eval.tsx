...
function canEval(connection: DebugConnection): boolean {
	return (
		...
		connection.capabilities.includes('eval')
	)
}
...
function canQuery(connection: DebugConnection): boolean {
	return (
		Array.isArray(connection.capabilities) &&
		connection.capabilities.includes('query')
	)
}
...
export function Eval({connection}: EvalProps) {
	...
	const out = useMemo(
		() => (res?.type === 'run-eval' ? res : undefined),
		[res]
	)
		...
		<Box flexDirection="column" flexGrow={1} paddingY={1}>
			...
			{!evalCapable ? (
				...
				>
					...
					{connection.status === 'connected' && !queryCapable && (
						<Text color={theme.muted}>
							{chars.arrow} Ensure you’re on the latest debug
							battery (engine auto-registers when
							UNRAG_DEBUG=true), or call{' '}
							<Text bold color={theme.fg}>
				...
				<>
					...
					>
						<Box
							...
							flexGrow={1}
							width={
								canSplit ? Math.floor(columns * 0.6) : undefined
							}
						>
							<Box
								...
								<Box flexDirection="column" gap={1}>
									...
									<Box flexDirection="column">
										...
										<Text color={theme.fg}>
											total{' '}
											{formatDuration(
												summary.timings.totalMs.p50
											)}
											/
											{formatDuration(
												summary.timings.totalMs.p95
											)}{' '}
											· retrieve{' '}
											{formatDuration(
												summary.timings.retrieveTotalMs
													.p50
											)}
											/
											{formatDuration(
									...
									{Array.isArray(summary.thresholdFailures) &&
										summary.thresholdFailures.length >
											0 && (
											<Box flexDirection="column">
												<Text
													color={theme.warning}
													bold
												>
													{chars.cross} threshold
													failures
												</Text>
												{summary.thresholdFailures
													.slice(0, 6)
													.map((f, i) => (
														<Text
															key={`${i}-${f}`}
						...
						<Box
							...
							flexGrow={1}
							width={
								canSplit ? Math.floor(columns * 0.4) : undefined
							}
						>
							...
							{!summary?.charts ? (
								...
								<Box flexDirection="column" gap={1}>
									<Box flexDirection="column">
										...
										<Sparkline
											data={
												summary.charts.retrievedRecall
											}
											width={Math.min(
												30,
												Math.max(
									...
									{summary.config.mode ===
										'retrieve+rerank' &&
										summary.charts.rerankedRecall &&
										summary.charts.rerankedMrr && (
											<>
												<Box flexDirection="column">
													<Text color={theme.muted}>
														reranked recall@k
