...
function inferLabel(opName: string, events: DebugEvent[]): string {
  const byType = (type: string) => events.find((e) => e.type === type) as any | undefined;
  ...
  if (opName === "retrieve") {
    ...
    return start?.query ? `q ${truncate(String(start.query), 52)}` : "retrieve";
  }
  if (opName === "rerank") {
    const start = byType("rerank:start");
    return start?.query ? `q ${truncate(String(start.query), 52)}` : "rerank";
  }
  if (opName === "delete") {
    const start = byType("delete:start");
    if (start?.mode && start?.value) return `${String(start.mode)} ${String(start.value)}`;
    return "delete";
  }
  return opName;
}

function computeStages(opName: string, events: DebugEvent[]): { totalMs?: number; stages: Stage[] } {
  const find = (type: string) => events.find((e) => e.type === type) as any | undefined;
  const stages: Stage[] = [];

  if (opName === "ingest") {
    const chunking = find("ingest:chunking-complete")?.durationMs as number | undefined;
    const embedding = find("ingest:embedding-complete")?.durationMs as number | undefined;
    const storage = find("ingest:storage-complete")?.durationMs as number | undefined;
    const total = find("ingest:complete")?.totalDurationMs as number | undefined;

    if (typeof chunking === "number") stages.push({ id: "chunking", label: "chunking", ms: chunking, color: theme.muted });
    if (typeof embedding === "number") stages.push({ id: "embedding", label: "embedding", ms: embedding, color: theme.ingest });
    if (typeof storage === "number") stages.push({ id: "storage", label: "storage", ms: storage, color: theme.borderActive });
  ...
  if (opName === "retrieve") {
    ...
    if (typeof embedding === "number") stages.push({ id: "embedding", label: "embedding", ms: embedding, color: theme.retrieve });
    if (typeof retrieval === "number") stages.push({ id: "db", label: "db", ms: retrieval, color: theme.borderActive });
    return { totalMs: total, stages };
  }
  ...
  if (opName === "rerank") {
    ...
    if (typeof rerankMs === "number") stages.push({ id: "rerank", label: "rerank", ms: rerankMs, color: theme.rerank });
    return { totalMs: total, stages };
  }
  ...
  if (opName === "delete") {
    ...
    if (typeof total === "number") stages.push({ id: "delete", label: "delete", ms: total, color: theme.delete });
    return { totalMs: total, stages };
  }
...
function StageBars({ stages, totalMs, width }: { stages: Stage[]; totalMs?: number; width: number }) {
  if (!stages.length) {
  ...
  return (
    <Box flexDirection="column" gap={0}>
      {stages.map((s) => {
        ...
        return (
          <Box key={s.id} gap={1}>
            <Text color={theme.muted}>{truncate(s.label.toUpperCase().padEnd(10), 10)}</Text>
            <Text color={theme.fg}>{formatDuration(s.ms).padStart(8)}</Text>
  ...
  return (
    <Box flexDirection="column" flexGrow={1} paddingY={1}>
      ...
      <Box flexDirection={canSplit ? "row" : "column"} flexGrow={1} gap={2}>
            ...
            traces.slice(scroll.windowStart, scroll.windowEnd).map((t, idx) => {
              ...
              return (
                <Box key={`${t.opId}-${t.startAt}`} gap={1}>
                  ...
                  <Text color={theme.muted}>{formatTime(t.startAt)}</Text>
                  <Text color={t.hasError ? theme.error : theme.fg} bold={isSel}>
                    {t.opName.toUpperCase()}
                  </Text>
                  <Text color={theme.muted}>·</Text>
                  <Text color={theme.fg}>{truncate(t.label, canSplit ? 54 : 42)}</Text>
                  {typeof t.totalMs === "number" && (
                    <Text color={theme.muted}>({formatDuration(t.totalMs)})</Text>
                  )}
                </Box>
              );
            })
        ...
        >
          {!selected ? (
            <Text color={theme.muted}>Select a trace to inspect.</Text>
          ) : (
            <>
              <Box marginBottom={1} gap={1}>
                ...
                <Text color={theme.muted}>
                  {selected.opId === "uncorrelated" ? "uncorrelated" : truncate(selected.opId, 16)}
                </Text>
              </Box>

              <StageBars stages={selected.stages} totalMs={selected.totalMs} width={canSplit ? 52 : 40} />
