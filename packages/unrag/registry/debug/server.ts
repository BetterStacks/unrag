...
type NodeWebSocketServer = {
	...
	on: (
		event: 'connection',
		handler: (ws: NodeWebSocketClient) => void
	) => void
}
...
export async function startDebugServer(
	...
	const handleOpen = (ws: WebSocketClient) => {
		...
		console.log(
			`[unrag:debug] Client connected (${clients.size}/${maxClients})`
		)
	}
	...
	const handleClose = (ws: WebSocketClient) => {
		...
		instance.clientCount = clients.size
		console.log(
			`[unrag:debug] Client disconnected (${clients.size}/${maxClients})`
		)
	}
