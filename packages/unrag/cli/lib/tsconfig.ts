...
export async function patchTsconfigPaths(params: {
	...
	const configFile = (await exists(
		path.join(params.projectRoot, 'tsconfig.json')
	))
		? 'tsconfig.json'
	...
	const parsedPaths =
		...
		typeof next.compilerOptions.paths === 'object'
			? next.compilerOptions.paths
			: {}
	next.compilerOptions.paths = {...parsedPaths}

	const hasBaseUrl =
		typeof next.compilerOptions.baseUrl === 'string' &&
		next.compilerOptions.baseUrl.length > 0
	const hasRagAlias = Array.isArray(next.compilerOptions.paths[aliasKey])
	const hasRagConfigAlias = Array.isArray(
		next.compilerOptions.paths[configAliasKey]
	)
