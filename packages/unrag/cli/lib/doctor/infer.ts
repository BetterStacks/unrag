...
export async function inferInstallState(options: {
	...
	const installDirFull = installDir
		? path.join(projectRoot, installDir)
		: null
	const installDirExists = installDirFull
		? await exists(installDirFull)
		: false

	// 4. Check config file
	const configFileExists = await exists(
		path.join(projectRoot, CONFIG_TS_FILE)
	)
	...
	if (unragJson?.embeddingProvider) {
		...
		embeddingProvider =
			await inferEmbeddingProviderFromFilesystem(installDirFull)
	}
	...
	try {
		...
		if (candidates.length === 0) {
			...
			for (const importPath of importedFiles) {
				...
				if (resolved) {
					const importContent = await readFile(
						resolved,
						'utf8'
					).catch(() => null)
					if (importContent) {
						candidates.push(
							...extractDbEnvVarCandidates(importContent)
						)
					}
				}
			}
				...
				if (paths) {
					for (const [alias, targets] of Object.entries(paths)) {
						...
						if (importPath.startsWith(aliasBase)) {
							const rest = importPath
								.slice(aliasBase.length)
								.replace(/^\//, '')
