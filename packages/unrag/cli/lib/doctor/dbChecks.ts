...
export async function runDbChecks(
	...
	try {
		...
		// Run schema checks
		const schemaResults = await checkSchema(
			client,
			options.schema,
			tableNames
		)
		...
		const uniquenessResult = await checkSourceIdUniqueness(
			client,
			options.schema,
			tableNames
		)
		...
		const duplicatesResult = await checkDuplicateSourceIds(
			client,
			options.schema,
			tableNames
		)
	...
	if (options.databaseUrlEnv) {
		...
		if (value) {
			return {
				...
				details: [
					`Using env var specified by flag: ${options.databaseUrlEnv}`
				]
			}
		}
		details.push(
			`${options.databaseUrlEnv} is not set (specified via --database-url-env).`
		)
	}
	...
	if (state.inferredDbEnvVar) {
		...
		if (value) {
			return {
				...
				details: [
					`Found ${state.inferredDbEnvVar} in your database configuration.`
				]
			}
		}
		details.push(
			`${state.inferredDbEnvVar} inferred from config but not set.`
		)
	}
	...
	try {
		...
		if (extResult.rows.length === 0) {
			return {
				...
				docsLink: docsUrl(
					'/docs/getting-started/database#enabling-pgvector'
				)
			}
		}
		...
		try {
			...
			return {
				...
				details: [
					'The <=> (cosine distance) operator may not be available.'
				]
			}
		}
	...
	try {
       ...
       WHERE tc.constraint_type = 'FOREIGN KEY'
		...
		const chunksFk = fks.find(
			(f) =>
				f.table_name === tableNames.chunks &&
				f.column_name === 'document_id'
		)
		if (!chunksFk) {
		...
		const embeddingsFk = fks.find(
			(f) =>
				f.table_name === tableNames.embeddings &&
				f.column_name === 'chunk_id'
		)
		if (!embeddingsFk) {
	...
	try {
       ...
       WHERE n.nspname = $1
		...
		return {
			...
			docsLink: docsUrl(
				'/docs/getting-started/database#schema-requirements'
			)
		}
	} catch (err) {
		const message = err instanceof Error ? err.message : String(err)
	...
	try {
       ...
       LIMIT 5`
		...
		return {
			...
			docsLink: docsUrl(
				'/docs/getting-started/database#resolving-duplicates'
			)
		}
	} catch (err) {
		const message = err instanceof Error ? err.message : String(err)
