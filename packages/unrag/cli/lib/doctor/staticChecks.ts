...
async function runInstallChecks(
  ...
  if (state.installDir) {
    ...
    const embeddingExists = await exists(path.join(installDirFull, "embedding"));
    const unragMdExists = await exists(path.join(installDirFull, "unrag.md"));
...
async function runEnvChecks(
  ...
  const dbUrl =
    ...
    (state.inferredDbEnvVar ? process.env[state.inferredDbEnvVar] : undefined);
  ...
  if (dbUrl) {
    ...
    results.push({
      ...
      details:
        state.inferredDbEnvVar && state.inferredDbEnvVar !== "DATABASE_URL"
          ? [
...
async function runModuleChecks(
  ...
  for (const extractor of state.installedExtractors) {
    ...
    const hasIndex =
      extractorExists && (await exists(path.join(extractorDir, "index.ts")));
  ...
  for (const connector of state.installedConnectors) {
    ...
    const hasIndex =
      connectorExists && (await exists(path.join(connectorDir, "index.ts")));
