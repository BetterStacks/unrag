...
export async function doctorSetupCommand(args: string[]): Promise<void> {
    ...
    : nonInteractive
      ...
      : await text({
          ...
          validate: (v) => {
            ...
            if (!v.endsWith(".json")) return "Config file must be .json";
            return;
          },
        });
  ...
  const dbEnvVarAnswer = nonInteractive
    ...
    : await text({
        ...
        validate: (v) => {
          ...
          if (!/^[A-Z_][A-Z0-9_]*$/i.test(v)) return "Invalid env var name";
          return;
        },
      });
  ...
  const skippedScripts = conflictingScripts.filter(
    (k) => !(k in scriptsToAdd) && !addedScripts.some((a) => a !== k && scripts[k])
  );
