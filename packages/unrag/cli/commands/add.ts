...
async function patchUnragConfig(args: {
	...
	try {
		...
		if (
			...
			!content.includes(
				`from "${installImportBase}/extractors/${args.extractor}"`
			)
		) {
				...
				options: [
					{
						value: 'keep',
						label: 'Keep existing',
						hint: existing[scriptName]
					},
					{
						value: 'overwrite',
						label: 'Overwrite',
						hint: desired[scriptName]
					},
					{
						value: 'rename',
...
export async function addCommand(args: string[]) {
	...
	if (!config?.installDir) {
		throw new Error(
			`Missing ${CONFIG_FILE}. Run \`unrag@latest init\` first.`
		)
	}
			...
			const sampleDataset = {
				...
				queries: [
					...
					},
					{
						...
						relevant: {
							sourceIds: ['eval:sample:doc:contact-support']
						}
					}
				]
			}
