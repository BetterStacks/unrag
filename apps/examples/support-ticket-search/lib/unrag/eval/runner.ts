...
export async function runEval(args: EvalRunArgs): Promise<EvalRunOutput> {
	...
	const datasetHasDocs =
		Array.isArray(dataset.documents) && dataset.documents.length > 0
	...
	for (const q of dataset.queries) {
		...
		const retrievedSourceIds = uniqueSourceIdsInOrder(
			retrieved.chunks.map((c) => c.sourceId)
		)
		...
		if (mode === 'retrieve+rerank') {
			...
			const rerankedSourceIds = uniqueSourceIdsInOrder(
				reranked.chunks.map((c) => c.sourceId)
			)
	...
	if (ingest && datasetHasDocs) {
		const shouldCleanup =
			cleanup === 'always' || (cleanup === 'on-success' && true) // errors would have thrown already
		if (shouldCleanup) {
			await args.engine.delete({sourceIdPrefix: scopePrefix})
		}
	}
	...
	// Aggregates
	const retrievedAgg = aggregatesFor(
		queryResults.map((q) => q.retrieved.metrics)
	)
	const rerankedAgg =
		mode === 'retrieve+rerank'
			? aggregatesFor(
					queryResults
						.map((q) => q.reranked?.metrics)
						.filter(Boolean) as EvalMetricsAtK[]
				)
			: undefined
	...
	const report: EvalReportV1 = {
		...
		engine: {
			embeddingModel,
			rerankerName: queryResults.find((q) => q.reranked)?.reranked?.meta
				?.rerankerName,
			rerankerModel: queryResults.find((q) => q.reranked)?.reranked?.meta
				?.model
		},
		results: {
...
function buildTimingAggregates(mode: EvalMode, qs: EvalQueryResult[]) {
	...
	const total = qs.map(
		(q) =>
			q.retrieved.durationsMs.totalMs +
			(q.reranked?.durationsMs.totalMs ?? 0)
	)
...
function evaluateThresholds(args: {
	...
	if (
		typeof min.hitAtK === 'number' &&
		args.aggregates.mean.hitAtK < min.hitAtK
	) {
		failures.push(
			`min.hitAtK: expected >= ${min.hitAtK}, got ${args.aggregates.mean.hitAtK.toFixed(3)}`
		)
	}
	if (
		typeof min.recallAtK === 'number' &&
		args.aggregates.mean.recallAtK < min.recallAtK
	) {
		failures.push(
			`min.recallAtK: expected >= ${min.recallAtK}, got ${args.aggregates.mean.recallAtK.toFixed(3)}`
		)
	}
	if (
		typeof min.mrrAtK === 'number' &&
		args.aggregates.mean.mrrAtK < min.mrrAtK
	) {
		failures.push(
			`min.mrrAtK: expected >= ${min.mrrAtK}, got ${args.aggregates.mean.mrrAtK.toFixed(3)}`
		)
	}
	if (
		typeof max.p95TotalMs === 'number' &&
		args.p95TotalMs > max.p95TotalMs
	) {
		failures.push(
			`max.p95TotalMs: expected <= ${max.p95TotalMs}, got ${args.p95TotalMs.toFixed(1)}ms`
		)
	}
