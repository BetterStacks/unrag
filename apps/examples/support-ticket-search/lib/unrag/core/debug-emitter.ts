...
import type {
	...
	DebugEventPayload,
	DebugEventType
} from '@unrag/core/debug-events'

/**
 * Payload for emitting a debug event (without timestamp and sessionId).
 */
export type DebugEventInput = DebugEventPayload<DebugEventType>
...
function createDebugEmitter(): DebugEmitter {
	...
	const _port = Number.parseInt(process.env.UNRAG_DEBUG_PORT ?? '3847', 10)
