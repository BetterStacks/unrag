...
<Callout>
The generated template sets all `assetProcessing` flags to `false` by default for cost safety. When you run `init --rich-media` and select extractors, the CLI enables the corresponding flags (e.g. `pdf.llmExtraction.enabled: true` if you select `pdf-llm`). If no extractor is registered for an enabled flag, ingestion emits warnings so you don't miss content silently.
</Callout>
