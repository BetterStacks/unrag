...
If you ran `init --rich-media`, the config includes additional extractor imports, a populated `extractors` array, `embedding.config.type: "multimodal"`, and enabled `assetProcessing` flags for your selected extractors.
...
Configuration for the embedding provider. If you ran `init --rich-media`, the CLI sets `type: "multimodal"` and picks a compatible model. See [AI SDK Embeddings](/docs/embedding/ai-sdk) and [Multimodal Embeddings](/docs/embedding/multimodal-embeddings) for more.
