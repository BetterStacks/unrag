...
export function createUnragEngine() {
  ...
  // const extractors = [createPdfLlmExtractor()];
  const extractors = [];
...
<Callout>
The generated template sets `pdf.llmExtraction.enabled: true`, but PDF extraction is performed by an extractor module. Install and register `pdf-llm` (or another PDF extractor) to actually process PDFs. If no extractor is registered, ingestion emits warnings so you don't miss content silently.
</Callout>
...
<Callout type="info">
Image handling (`image:embed`, `image:caption`) is built into the core engine—no extractor module needed. Configure via your embedding provider's `type` setting.
</Callout>
