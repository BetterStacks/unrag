...
Install the Gemini PDF extractor:

```bash
bunx unrag add extractor pdf-llm --yes
```

Then register it in `createUnragEngine()` by importing it and adding it to the engine’s `extractors` array.
