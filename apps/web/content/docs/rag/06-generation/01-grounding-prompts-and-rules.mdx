...
<Accordions>
<Accordion title="Rule 1: Answer from provided context">

The model should answer from the provided context. A clear instruction like "Answer the user's question using only the information provided in the sources below. Do not use information from your training data unless it is confirmed by the sources" establishes this expectation.

</Accordion>

<Accordion title="Rule 2: Cite sources">

The model should cite sources. Specify the format: "Cite sources using [1], [2], etc. Every factual claim should have a citation." This makes answers verifiable and helps users trust the response.

See [Context packing and citations](/docs/rag/05-reranking-and-context/05-context-packing-and-citations) for implementation patterns.

</Accordion>

<Accordion title="Rule 3: Acknowledge limitations">

The model should acknowledge limitations. "If the sources don't contain enough information to answer the question, say so. Don't guess or make up information." This is crucial for preventing hallucinations.

See [topK and thresholds](/docs/rag/04-retrieval/01-topk-thresholds-and-no-good-match) for handling no-match cases.
...
**With Unrag**: See [Use with chat](/docs/guides/use-with-chat) for complete prompt examples.
</Callout>
