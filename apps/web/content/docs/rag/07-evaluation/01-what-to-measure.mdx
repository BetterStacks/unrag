...
**Recall@K** measures whether the relevant documents appear anywhere in the top K results. If the correct document is in positions 1-10, recall@10 is 1; if it's at position 15, recall@10 is 0. This tells you whether relevant content is being retrieved at all.

**Precision@K** measures what fraction of the top K results are relevant. If 3 of the top 5 are relevant, precision@5 is 0.6. High precision means less noise in context; low precision means you're including irrelevant content.

**Mean Reciprocal Rank (MRR)** measures where the first relevant result appears. If the first relevant result is at position 3, its reciprocal rank is 1/3. MRR averages this across queries. Higher MRR means relevant content appears earlier.

**nDCG (Normalized Discounted Cumulative Gain)** accounts for graded relevance (some documents are more relevant than others) and position. It rewards having highly relevant documents ranked early.
