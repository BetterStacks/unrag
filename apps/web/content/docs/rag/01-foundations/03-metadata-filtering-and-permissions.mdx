---
title: "Metadata, filtering, and permissions"
...
**With Unrag**: Attach metadata via `engine.ingest({ metadata: {...} })` and filter via `engine.retrieve({ scope: {...} })`. See [metadata and scoping](/docs/concepts/metadata-and-scoping) for patterns, and the [multi-tenant guide](/docs/guides/multi-tenant) for tenant isolation.
