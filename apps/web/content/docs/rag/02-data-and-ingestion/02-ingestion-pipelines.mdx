...
**With Unrag**: Run a script that calls `engine.ingest()` on a schedule. See [admin reingest example](/docs/examples/admin-reingest) for patterns.
...
**With Unrag**: Use consistent `sourceId` values when calling `engine.ingest()`. The same `sourceId` replaces existing content rather than duplicating it. See [document modeling](/docs/rag/02-data-and-ingestion/04-document-modeling-and-stable-ids) for best practices.
