...
**With Unrag**: Use `engine.retrieve()` with a higher `topK` value to retrieve candidates. See [ANN search](/docs/rag/01-foundations/04-indexing-and-ann-search).
...
**With Unrag**: Use the built-in [reranker](/docs/batteries/reranker) powered by Cohere's rerank API, or implement custom reranking.
