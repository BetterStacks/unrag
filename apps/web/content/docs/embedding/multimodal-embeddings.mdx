...
1. **Bytes**: Image bytes are passed directly to the embedding provider
2. **URLs**: The URL is fetched server-side using [`assetProcessing.fetch`](/docs/reference/asset-processing#fetchconfig) settings, then the resulting bytes are passed to the provider
3. The provider calls the multimodal model's image embedding endpoint
4. A vector is returned representing the image's semantic content
5. This vector is stored alongside text chunk vectors
...
See [Performance](/docs/concepts/performance) for more details on tuning embedding throughput.
