...
<Callout>
The fastest way to enable multimodal embeddings is during setup. Run `bunx unrag@latest init --rich-media` and the CLI configures your embedding provider with multimodal support. If you've already run `init`, you can re-run with `--rich-media` to enable it, or follow the manual steps below.
</Callout>
