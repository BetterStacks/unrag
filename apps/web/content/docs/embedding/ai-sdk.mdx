...
**`type`** controls capabilities:

- `text` (default): text embeddings only
- `multimodal`: additionally exposes `embedImage()` for image assets (when the selected model/provider supports it)
...
When `type: "multimodal"` is enabled and you do not specify a model (and `AI_GATEWAY_MODEL` is not set), Unrag defaults to `voyage/voyage-multimodal-3`.
...
```ts
{
  ...
  embedMany: async (inputs) => number[][],
  // embedImage: async (input) => number[] // only when type: "multimodal"
}
```
