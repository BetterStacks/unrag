...
The function returns a small summary so you can log results or surface them in an admin UI: `pageCount`, `succeeded`, `failed`, `deleted`, and `errors`.

#### Required parameters

`engine` is a `ContextEngine` from `createUnragEngine()`. This is where the connector ingests content.

`token` is your Notion integration token, usually read from an environment variable.

`pageIds` is an array of page IDs or page URLs. The connector accepts both formats and normalizes them internally.
...
This lower-level helper loads a single page and returns a normalized document shape with `sourceId`, `content`, and `metadata`. Use it when you want to add custom metadata, control chunking, or decide exactly how ingestion happens (batching, retries, conditional writes).
...
Creates a Notion API client. Most users don't need this unless they want to build custom fetch logic on top of the connector.
