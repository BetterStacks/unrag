...
3. **defaults**: Default chunking parameters (`chunkSize` and `chunkOverlap`)
4. **chunker**: A custom function for splitting documents (optional)
5. **idGenerator**: A custom function for generating UUIDs (optional, defaults to `crypto.randomUUID()`)
