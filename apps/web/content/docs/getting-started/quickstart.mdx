...
Your quickstart used plain text content, but Unrag can handle more. The generated `unrag.config.ts` includes `assetProcessing` settings that enable:

- **PDF extraction**: Send PDFs to an LLM to extract their text content
- **Image embedding**: Embed images directly (with multimodal models) or use their captions
