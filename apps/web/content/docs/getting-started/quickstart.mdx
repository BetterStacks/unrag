...
This presents a list of extractors to install (PDF, image, audio, video, files) and configures your `unrag.config.ts` with multimodal embeddings enabled. The CLI handles all the wiring—registering extractors, enabling the right `assetProcessing` flags, and switching your embedding provider to a multimodal model.
