...
For a comprehensive guide to RAG architecture—including the quality-latency-cost tradeoffs at each stage, common failure modes, and production considerations—see the [RAG Handbook](/docs/rag). The [Orientation](/docs/rag/0-orientation) module covers the two-pipeline mental model in depth.
</Callout>
