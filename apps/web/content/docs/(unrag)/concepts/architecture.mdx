---
title: Architecture
description: How the ingest, retrieve, delete, and rerank pipelines work from end to end.
---
...
Reranking is optional and requires installing the reranker battery. The default implementation uses Cohere's rerank-v3.5 model, but you can bring your own reranker. See the [Reranker documentation](/docs/batteries/reranker) for details.
...
<Callout title="Deep dive: RAG pipelines">
For a comprehensive guide to RAG architecture—including the quality-latency-cost tradeoffs at each stage, common failure modes, and production considerations—see the [RAG Handbook](/docs/rag). The [Orientation](/docs/rag/00-orientation) module covers the two-pipeline mental model in depth.
</Callout>
