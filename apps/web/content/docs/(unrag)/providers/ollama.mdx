...
This starts the Ollama server on `http://localhost:11434`. The server loads models on demand—the first embedding request for a model will be slower while the model loads into memory.
