...
When a tenant is deleted or requests data removal, use prefix deletion to clean up all their content:

```ts
// Delete all content for a tenant
await engine.delete({ sourceIdPrefix: `tenant:${tenantId}:` });

// Delete a specific workspace within a tenant
await engine.delete({
  sourceIdPrefix: `tenant:${tenantId}:workspace:${workspaceId}:`
});
```

The prefix-based deletion removes all matching documents and their associated chunks and embeddings in a single operation, making tenant offboarding straightforward.
...
Tenant isolation in Unrag happens at the query level, not the database level. A bug in your application code could expose one tenant's data to another.
</Callout>
...
For deeper coverage of tenant isolation patterns and scaling strategies, see [Scaling and multi-tenancy](/docs/rag/08-production-and-ops/07-scaling-and-multi-tenancy) in the RAG Handbook.
