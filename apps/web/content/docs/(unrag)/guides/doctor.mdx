...
**Module presence.** If you've installed extractors, connectors, or chunkers, doctor verifies that the source files exist in the expected locations. A module directory without its main files suggests a partial installation.

**Config coherence.** Doctor performs static analysis on your `unrag.config.ts` to check whether installed extractors are actually registered and enabled, and whether your configured chunking method is correctly wired (plugin installed + imported). If you've installed `pdf-llm` but didn't add it to your extractors array, doctor warns you.

**API feature checks.** Doctor can verify that your vendored engine sources include newer API capabilities (for example, support for per-ingest `engine.ingest({ chunker })` overrides) without running your app.
