...
Asset processing is opt-in and configurable. The library defaults to skipping expensive operations, and rich media settings are only enabled when you choose extractors during `init` (or add them later). See [Multimodal Ingestion](/docs/concepts/multimodal) for the full picture.
