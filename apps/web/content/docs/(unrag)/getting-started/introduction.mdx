...
**`lib/unrag/embedding/`** contains implementations for all supported embedding providers—Vercel AI Gateway (default), OpenAI, Google, Cohere, Voyage, and others. The default uses the Vercel AI SDK, but you can switch providers by changing a config option. All providers are installed so you can swap without re-running the CLI.
...
**Server-side only.** Unrag reads your database credentials and embedding API keys from environment variables. It's designed to run in Route Handlers, Server Actions, API routes, or backend scripts—never in browser code. This also means edge runtimes (Cloudflare Workers, Vercel Edge) aren't supported out of the box. See [Supported Runtimes](/docs/getting-started/supported-runtimes) for the full picture on where Unrag runs.
...
<Callout title="New to RAG?">
If you're new to retrieval-augmented generation or want a deeper understanding of the concepts, tradeoffs, and failure modes involved in production RAG systems, check out our comprehensive [RAG Handbook](/docs/rag). It covers everything from first principles to production operations.
</Callout>
