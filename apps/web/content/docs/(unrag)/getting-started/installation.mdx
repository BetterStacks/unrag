...
If you enabled rich media during setup, you'll also see additional extractor modules under the `extractors/` folder for the specific extractors you selected (e.g., `pdf-text-layer/`, `image-ocr/`).
...
**`lib/unrag/`** contains the vendored module code including the core engine (with ingest, retrieve, delete, and rerank operations), your chosen store adapter, all embedding provider implementations, and shared utilities. These files are yours to read and modify.
