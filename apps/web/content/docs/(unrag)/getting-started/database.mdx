...
<Tabs items={['Local/Self-hosted', 'Neon', 'Supabase', 'AWS RDS', 'Azure', 'Google Cloud']}>
<Tab value="Local/Self-hosted">
Run the SQL directly, or add it to your migration files. You may need to install the pgvector extension first—on Ubuntu, that's `apt install postgresql-15-pgvector` (adjust for your Postgres version).
</Tab>
<Tab value="Neon">
pgvector is pre-installed. Just run `create extension if not exists vector;` and you're ready.
</Tab>
<Tab value="Supabase">
pgvector is available in the dashboard under **Database → Extensions**. Enable it there, or run the SQL in the SQL Editor.
</Tab>
<Tab value="AWS RDS">
pgvector is available on RDS for PostgreSQL 15.2+. You enable it with the same `create extension` command.
</Tab>
<Tab value="Azure">
Enable pgvector through the Azure portal under **Server parameters**, then run the extension command.
</Tab>
<Tab value="Google Cloud">
pgvector is supported on Cloud SQL for PostgreSQL. Enable it via the Google Cloud Console or `gcloud` CLI.
</Tab>
</Tabs>
...
The `documents` table represents logical documents you've ingested. The `source_id` is your identifier for the document—something like `docs:getting-started` or `kb:article-123`. When you re-ingest content with the same `source_id`, Unrag updates the existing document. The `content` column stores the full original text (useful for debugging and re-chunking) and can be disabled via `storage.storeDocumentContent` (Unrag will store an empty string). `metadata` holds any JSON you passed during ingestion.
...
The `chunks` table holds the individual pieces that documents are split into. Each chunk belongs to a document (`document_id`), has an index showing its position in the original document (`idx`), contains the chunk's text (`content`), and records an approximate token count. You can disable persisting chunk text via `storage.storeChunkContent` (retrieval still works, but `chunk.content` will be empty). The `source_id` is denormalized here for faster filtering during retrieval.
...
Once you've created the schema and configured your environment, you can verify everything is working with the doctor command:

```bash
bunx unrag doctor --db
```

This connects to your database and checks that pgvector is enabled, the tables exist with the expected columns, foreign keys have cascade deletes, and recommended indexes are present. If anything is misconfigured, doctor tells you what's wrong and how to fix it.

For more on using doctor, see the [Validating Your Installation](/docs/guides/doctor) guide.
