...
<TypeTable
  type={{
    ...
    chunking: {
      description: 'Optional per-call chunking override (chunkSize, chunkOverlap, minChunkSize). All sizes are in tokens.',
      type: 'object',
    },
    chunker: {
      description:
        'Optional per-call chunker override. Use this to switch chunking algorithms for a single ingest() call without changing your engine config.',
      type: 'Chunker',
    },
    assets: {
      description:
        'Optional rich media inputs (images, PDFs, audio, etc.) that can be processed into chunks. Connectors like Notion and Google Drive populate this for you.',
      type: 'AssetInput[]',
      default: '[]',
    },
    assetProcessing: {
...
The `stage` field in `asset_processing_error` indicates where the failure occurred:

| Stage | Description |
|-------|-------------|
| `fetch` | Failed to fetch URL-based asset data (applies to extractors and image embedding) |
| `extract` | Extractor threw an error while processing asset bytes |
| `embed` | Embedding provider threw an error |
| `unknown` | Fallback for unexpected error locations |
...
<TypeTable
  type={{
    ...
    chunker: {
      description: 'Optional custom chunking function. Defaults to token-based recursive splitter using o200k_base encoding.',
      type: 'Chunker',
    },
    idGenerator: {
