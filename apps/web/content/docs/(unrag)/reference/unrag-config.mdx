...
- Registers it in the `extractors` array
- Adds minimal `assetProcessing` overrides (only the `enabled: true` flags needed for that extractor)
...
Controls how documents are split into chunks. Unrag uses token-based recursive chunking with the `o200k_base` tokenizer (same as GPT-5, GPT-4o). See [Chunking](/docs/concepts/chunking) for details on strategies and plugin chunkers.

<TypeTable
  type={{
    chunkSize: {
      description: 'Maximum tokens per chunk. Smaller = more precise retrieval, higher embedding cost.',
      type: 'number',
      default: '512',
    },
    chunkOverlap: {
      description: 'Tokens that overlap between adjacent chunks. Preserves context across boundaries.',
      type: 'number',
      default: '50',
    },
    minChunkSize: {
      description: 'Minimum tokens per chunk. Chunks smaller than this will be merged with neighbors.',
      type: 'number',
      default: '24',
    },
  }}
...
<TypeTable
  type={{
    concurrency: {
      description:
        'Max number of concurrent embedding requests during ingest. This applies to both text embedding (embed/embedMany) and image embedding (embedImage).',
      type: 'number',
      default: '4',
    },
    batchSize: {
      description:
        'Max number of text chunks per embedMany batch (only used when the provider supports embedMany).',
      type: 'number',
      default: '32',
    },
  }}
...
<TypeTable
  type={{
    method: {
      description: 'Chunking method to use. Built-in: "recursive" (default), "token". Plugins: "semantic", "markdown", "hierarchical", "code", "agentic". Use "custom" with a chunker function.',
      type: '"recursive" | "token" | "semantic" | "markdown" | "hierarchical" | "code" | "agentic" | "custom"',
      default: '"recursive"',
    },
    options: {
      description: 'Chunking options (chunkSize, chunkOverlap, minChunkSize). Same as defaults.chunking.',
      type: 'ChunkingOptions',
    },
    chunker: {
      description: 'Custom chunker function. Only used when method is "custom".',
      type: 'Chunker',
    },
  }}
/>
...
This:
- Copies the extractor source to `lib/unrag/extractors/pdf-llm/`
- Adds dependencies to your `package.json`
- **Automatically patches `unrag.config.ts`** to:
  - Add the extractor import
  - Register it in the `extractors` array
  - Enable the corresponding `assetProcessing` flags (minimal overrides only)
