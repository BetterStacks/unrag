...
|--------|-----------------|----------------|-------------|
| `pdf-text-layer` | `unrag add extractor pdf-text-layer` | `pdf:text-layer` | Extract built-in PDF text layer (fast/cheap) |
| `pdf-llm` | `unrag add extractor pdf-llm` | `pdf:llm` | Extract text from PDFs using an LLM |
| `pdf-ocr` | `unrag add extractor pdf-ocr` | `pdf:ocr` | OCR PDFs by rasterizing pages (worker-only) |
| `image-ocr` | `unrag add extractor image-ocr` | `image:ocr` | OCR images into searchable text |
| `image-caption-llm` | `unrag add extractor image-caption-llm` | `image:caption-llm` | Generate image captions via LLM |
| `audio-transcribe` | `unrag add extractor audio-transcribe` | `audio:transcribe` | Transcribe audio into text chunks |
| `video-transcribe` | `unrag add extractor video-transcribe` | `video:transcribe` | Transcribe video audio track into text chunks |
| `video-frames` | `unrag add extractor video-frames` | `video:frames` | Sample frames + extract text per frame (worker-only) |
| `file-text` | `unrag add extractor file-text` | `file:text` | Decode text-ish files (txt/md/html/json/csv) |
| `file-docx` | `unrag add extractor file-docx` | `file:docx` | Extract raw text from `.docx` |
| `file-pptx` | `unrag add extractor file-pptx` | `file:pptx` | Extract slide text from `.pptx` |
| `file-xlsx` | `unrag add extractor file-xlsx` | `file:xlsx` | Extract sheet content from `.xlsx` |
