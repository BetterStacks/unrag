...
2. **For URLs**: The URL is fetched server-side using [`assetProcessing.fetch`](/docs/reference/asset-processing#fetchconfig) settings, then the bytes are passed to the model
